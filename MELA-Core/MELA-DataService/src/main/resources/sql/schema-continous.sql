...
create CACHED table IF NOT EXISTS StructuredCollectedData (ID int IDENTITY, monSeqID VARCHAR(200), timestampID int, data OTHER, FOREIGN KEY (monSeqID) REFERENCES MonitoringSeq(ID), FOREIGN KEY (timestampID) REFERENCES Timestamp(ID) )
...
create CACHED table IF NOT EXISTS EVENTS (ID int IDENTITY PRIMARY KEY, monSeqID VARCHAR(200), event VARCHAR(200), flag VARCHAR(10))
