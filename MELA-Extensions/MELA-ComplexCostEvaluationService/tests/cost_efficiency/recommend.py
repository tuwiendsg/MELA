...
#
#########################################
import os, sys, httplib, uuid, random, time, json, subprocess, StringIO, datetime
...
def evaluateAndPersistCostEfficiencyForScalingAllStrategies():
 if len(listOfEventProcessingIPs) > 1:
   ipSStringForMela=""
   for strategy in listOfEventProcessingIPs:
      ipSStringForMela = ipSStringForMela + "-" + strategy
      ipSStringForMela =  ipSStringForMela + "-" getIPToScaleIn(STRATEGY_MELA_COST_RECOMMENDATION_LIFETIME)
      ipSStringForMela =  ipSStringForMela + "-" getIPToScaleIn(STRATEGY_MELA_COST_RECOMMENDATION_EFFICIENCY)
     #remove first -
   ...
   print efficiencyCSVLine
   f = open("./efficiency_try.csv", "a+")
