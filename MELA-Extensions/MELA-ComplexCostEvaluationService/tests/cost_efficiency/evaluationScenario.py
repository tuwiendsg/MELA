...
eventProcessingStartIP="10.99.0.40"
loadBalancerStartIP="10.99.0.25"
...
serviceId= "EventProcessingTopologyCE"
...
def evaluateAndPersistCostEfficiencyForScalingAllStrategies():
   ...
   if len(listOfEventProcessingIPs) > 1:
     ...
     if maximumIPs < len(listOfEventProcessingIPs):
         maximumIPs = len(listOfEventProcessingIPs)
     ipSStringForMela=""
     
     for strategy in strategiesList:
        print "ReturnedIP for " + strategy + " is " + getIPToScaleIn(strategy)
        ip = getIPToScaleIn(strategy)
        ipSStringForMela = ipSStringForMela + "-" + ip

     for ip in listOfEventProcessingIPs:	
        ipSStringForMela = ipSStringForMela + "-" + ip
     ...
     efficiencyCSVLine = "" + str(int(time.time())) + "," + str(datetime.datetime.now()) + "," + str(len(listOfEventProcessingIPs))
     ...
     if maximumIPs > len(listOfEventProcessingIPs):
        for i in range(0, maximumIPs -len(listOfEventProcessingIPs)):
...
def scaleOutAndInchreaseLoad(proc):
    ...
    if proc:
       proc.terminate()
       proc = subprocess.Popen(["exec python ./load.py " + loadBalancerStartIP + " " + str(len(listOfEventProcessingIPs) * 90)], shell=True)
...
if __name__=='__main__':
     ...
     updateMELAServiceDescriptionAfterScaleOut("10.99.0.15")
     ...
     #  p.join()
     time.sleep(500) #wait so all VMS have been running for one hour hour
     ...
     #  #check every 1 minutes, and sleep 10, so total wait 10 minutes
     #  for j in range(0,10):
     #     #evaluate and persist
     #     print "Evaluating after scale out" + str(j)
     #     evaluateAndPersistCostEfficiencyForScalingAllStrategies()
     #     time.sleep(60)
     for i in range(0, 5):
       print "Scale in"
       ...
       for j in range(0,7):
          print "Evaluating after scale in " + str(j)
          evaluateAndPersistCostEfficiencyForScalingAllStrategies()
          time.sleep(60)
       scaleOutAndInchreaseLoad(proc)
       #check every 1 minutes, and sleep 10, so total wait 10 minutes
