...
create CACHED table IF NOT EXISTS CaschedCompleteHistoricalUsage (ID int IDENTITY PRIMARY KEY, monSeqID VARCHAR(200), timestampID int, data  OTHER, FOREIGN KEY (monSeqID) REFERENCES MonitoringSeq(ID), FOREIGN KEY (timestampID) REFERENCES Timestamp(ID) );
...
create CACHED table IF NOT EXISTS InstantCostElasticityPathway (ID int IDENTITY PRIMARY KEY, monSeqID VARCHAR(200), timestampID int, elasticityPathway OTHER, FOREIGN KEY (monSeqID) REFERENCES MonitoringSeq(ID), FOREIGN KEY (timestampID) REFERENCES Timestamp(ID) )
